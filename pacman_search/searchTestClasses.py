...
class GraphSearch(searchProblems.SearchProblem):
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search, moduleDict):
        
        searchProblems = moduleDict['searchProblems']
        searchHeuristics = moduleDict['searchHeuristics']
        ...
        problemClass = getattr(searchProblems, self.searchProblemClassName)
        problemOptions = {}
        if self.costFn != None:
            problemOptions['costFn'] = self.costFn
        problem = problemClass(start_state, **problemOptions)
        heuristic = getattr(searchHeuristics, self.heuristicName) if self.heuristicName != None else None
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        solution, expanded, error = self.getSolInfo(search, moduleDict)
        if error != None:
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage('%s' % error)
            return False
...
class CornerProblemTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents):
        ...
        problem = searchProblems.CornersProblem(gameState)
...
class HeuristicTest(testClasses.TestCase):
    ...
    def setupProblem(self, moduleDict):
        searchProblems = moduleDict['searchProblems']
        searchHeuristics = moduleDict['searchHeuristics']
        lay = layout.Layout([l.strip() for l in self.layoutText.split('\n')])
        gameState = pacman.GameState()
        ...
        state = problem.getStartState()
        heuristic = getattr(searchHeuristics, self.heuristicName)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        problem, state, heuristic = self.setupProblem(moduleDict)
...
class HeuristicGrade(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        search = moduleDict['search']
        searchAgents = moduleDict['searchAgents']
        problem, _, heuristic = self.setupProblem(moduleDict)
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        problem = searchProblems.CornersProblem(game_state)
        start_state = problem.getStartState()
        h0 = searchHeuristics.cornersHeuristic(start_state, problem)
        succs = problem.getSuccessors(start_state)

        heuristic_cost = searchHeuristics.cornersHeuristic(start_state, problem)
        ...
        heuristics = []
        for state in states:
            heuristics.append(searchHeuristics.cornersHeuristic(state, problem))
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        problem = searchProblems.CornersProblem(game_state)
        start_state = problem.getStartState()
        if searchHeuristics.cornersHeuristic(start_state, problem) > true_cost:
            grades.addMessage('FAIL: Inadmissible heuristic')
            return False
        path = search.astar(problem, searchHeuristics.cornersHeuristic)
        print("path:", path)
